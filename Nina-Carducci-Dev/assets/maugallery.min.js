...
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i}});next=imagesCollection[index]||imagesCollection[0];$(".lightboxImage").attr("src",$(next).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
          ...
          <span class="nav-link"  data-images-toggle="${value}">${value}</span></li>`});var tagsRow=`<ul class="my-4 tags-bar nav nav-pills">${tagItems}</ul>`;if(position==="bottom"){gallery.append(tagsRow)}else if(position==="top"){gallery.prepend(tagsRow)}else{console.error(`Unknown tags position: ${position}`)}},filterByTag(){var tag=$(this).data("images-toggle");if(tag==="all"){$(".nav-link").removeClass("active");$(this).addClass("active");$(".item-column").show(300)}else{$(".nav-link").removeClass("active");$(this).addClass("active");$(".item-column").hide();$(".gallery-item").filter(function(){return $(this).data("gallery-tag")===tag}).parents(".item-column").show(300)}}}})(jQuery)